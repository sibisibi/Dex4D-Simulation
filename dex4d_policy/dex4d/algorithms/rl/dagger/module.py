...
class ActorPointNetTransformerV2(nn.Module): # student
    '''
    Changes:
    - For each concatenation of (object_kp, goal_kp), use a shared MLP to embed it into a token
    - Use a separate learnable [CLS] token to the input and use the output of the [CLS] token for action prediction
    - No positional encoding (Linear layer bias can provide positional info)
    '''

    def __init__(self, obs_shape, states_shape, actions_shape, initial_std, model_cfg, asymmetric=False, use_pc = False):
        super(ActorPointNetTransformerV2, self).__init__()
    ...
    def build_tokenizers(self):
        ...
        assert sum(self.obs_dim_dict.values()) == self.num_obs, "observation dim does not match!"
        self.tokenizers = nn.ModuleDict()
        for key, dim in self.obs_dim_dict.items():
            if "kp" not in key:
                self.tokenizers[key] = nn.Linear(dim, self.token_dim)
            else:
                self.tokenizers[key] = nn.Linear(6, self.token_dim)
    ...
    def tokenize(self, observations):
        ...
        for i, (key, value) in enumerate(self.obs_dim_dict.items()):
            if "kp" not in key:
                ...
                goal_kp = obs[:, self.num_keypoints:, :] # [B, self.num_keypoints, 3]
                concatenated_kp = torch.cat([object_kp, goal_kp], dim=-1) # [B, self.num_keypoints, 6]
                token = self.tokenizers[key](concatenated_kp) # [B, self.num_keypoints, token_dim]
                tokens.append(token)
    ...
    def act(self, observations):
        tokens = self.tokenize(observations)
        actions = self.actor(tokens, output_token_idx=0) # use [CLS] token
        return actions
