...
###################################################################################
# NOTE: This class (ActorCriticPointNetTransformer) is highly dependent on the   #
# env state and observation structure                                             #
###################################################################################

class ActorCriticPointNetTransformer(nn.Module):

    def __init__(self, obs_shape, states_shape, actions_shape, initial_std, model_cfg, asymmetric=False, use_pc = False):
        super(ActorCriticPointNetTransformer, self).__init__()
        ...
        self.actor = Transformer(input_dim=self.token_dim, output_dim=actions_shape[0], latent_dim=self.token_dim, num_layer=4, use_input_layer=False, use_positional_encoding=False)
        self.actor.output_layer[1].weight.data *= 0.01 # init last layer to be 100x smaller
        self.critic = Transformer(input_dim=self.token_dim, output_dim=1, latent_dim=self.token_dim, num_layer=4, use_input_layer=False, use_positional_encoding=False)
    ...
    def build_tokenizers(self):
        self.full_state_dim_dict = {
            'robot_finger_hand': 66 + 76 + 13,
            'action': 22,
            'object_state_goal_pose': 13 + 7,
            'object_and_goal_kp': self.num_keypoints * 6,
            'pcd_feat_ftobj': 64 + 12,
        }
        self.output_token_idx = 1 # use action token
        assert sum(self.full_state_dim_dict.values()) == self.num_states, "full state dim does not match!"
        self.tokenizers = nn.ModuleDict()
        for key, dim in self.full_state_dim_dict.items():
            if "kp" not in key:
                self.tokenizers[key] = nn.Linear(dim, self.token_dim)
            else:
                self.tokenizers[key] = PointNetBackbone(pc_dim=6, feature_dim=self.token_dim)
        
    def tokenize(self, observations, states):
        assert observations.shape == states.shape, "Assume symmetric observation and state shapes"
        states_ptr = 0
        tokens = []
        for i, (key, value) in enumerate(self.full_state_dim_dict.items()):
            if "kp" not in key:
                state = states[:, states_ptr:states_ptr+value]
                states_ptr += value
                token = self.tokenizers[key](state) # [B, token_dim]
                token = token.unsqueeze(1) # [B, 1, token_dim]
                tokens.append(token)
            else:
                state = states[:, states_ptr:states_ptr+value].reshape(-1, value//3, 3) # [B, self.num_keypoints*2, 3]
                states_ptr += value
                object_kp = state[:, :self.num_keypoints, :] # [B, self.num_keypoints, 3]
                goal_kp = state[:, self.num_keypoints:, :] # [B, self.num_keypoints, 3]
        ...
        tokens = torch.cat(tokens, dim=1) # [B, num_tokens, token_dim]
    ...
    def act(self, observations, states):
        tokens = self.tokenize(observations, states)

        actions_mean = self.actor(tokens, self.output_token_idx)
        ...
        if self.asymmetric:
            value = self.critic(tokens, self.output_token_idx)
        else:
            value = self.critic(tokens, self.output_token_idx)
    ...
    def act_inference(self, observations):
        tokens = self.tokenize(observations, observations)
        actions_mean = self.actor(tokens, self.output_token_idx)
        return actions_mean.detach()
