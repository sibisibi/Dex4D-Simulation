...
class BaseTask():
    ...
    def __init__(self, cfg, enable_camera_sensors=False, is_meta=False, task_num=0):
        ...
        self.context_length = cfg["env"].get("contextLength", 1)
        ...
        self.obs_history_buf = torch.zeros(
            (self.num_envs, self.context_length, self.num_obs), device=self.device, dtype=torch.float)
        self.states_buf = torch.zeros(
            (self.num_envs, self.num_states), device=self.device, dtype=torch.float)
        self.states_history_buf = torch.zeros(
            (self.num_envs, self.context_length, self.num_states), device=self.device, dtype=torch.float)
    ...
    def step(self, actions, id):
        ...
        for i in range(self.control_freq_inv):
            ...
            if getattr(self, "is_bench", False):
                if getattr(self, "self_record_video", False):
                    self.video_recorder.record_frame()
                if getattr(self, "use_cotracker", False) and i % 2 == 0:
                    self._update_cotracker_keypoints()
                    self._apply_cotracker_keypoints_to_obs()
                    pass
