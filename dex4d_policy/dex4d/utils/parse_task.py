...
from tasks.fr3_xhand_ap2ap_vision import FR3XHandAP2APVision
...
def parse_task(args, cfg, cfg_train, sim_params, agent_index):
    ...
    cfg['test'] = args.test
    cfg['vis_env_id'] = args.vis_env_id
    ...
    if args.task_type == "C++":
        ...
        if args.task == "OneFrankaCabinet" :
            env = VecTaskPythonArm(task, rl_device, clip_actions=cfg_train.get("clip_actions", 1.0))
        else :
            env = VecTaskPython(task, rl_device, clip_actions=cfg_train.get("clip_actions", 1.0))
    return task, env
