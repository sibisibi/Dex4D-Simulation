#!/bin/bash
# Stock Dex4D teacher PPO stage 3 (ALLCAT, 5 Hz), continues a stage 1-2 final
# checkpoint from iteration 25000 to 50000 (iteration parsed from the filename).
# Usage: launch_dex4d_default_stage3.sh <GPU> <STAGE12_FINAL_CKPT>
set -e
cd "$(dirname "$0")/.."
...
LOG=logs/xarm6_leap_hand_ap2ap/ppo/dex4d-default-stage3_$ts
mkdir -p "$LOG"
