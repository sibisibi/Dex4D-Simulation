...
# Usage: launch_dex4d_fr3xhand_s1s2.sh <GPU> <STAGE1_ITERS> <STAGE2_ITERS> <ARM_GAIN_ROW>
...
LOG=logs/fr3_xhand_ap2ap/ppo/dex4d-fr3xhand-${ROW}-s1-${S1}-s2-${S2}_$ts
mkdir -p "$LOG"
...
python train.py --task=FR3XHandAP2AP --algo=ppo --seed=0 \
  ...
  --stage2_start_iteration=$S1 --arm_gain_row=$ROW \
