"""Dex4D's object and goal reset, engine agnostic.

`reset_object_pose` at :1090 and `reset_goal_pose` at :1124, with the simulator
writes left to the caller so this stays pure. The `goal_cond` prior branch is not
ported, it is asserted off for FR3 plus XHand at :335.

Three things carried across deliberately, all of them surprising.

The first goal of an episode is the object raised by `goal_displacement` with its
orientation COPIED, so the opening reorientation demand is exactly zero. It is
also the one path that never passes through the clamp.

The forced-flat goals write `object_init_state[:, 2]`, which is the object's
spawn height of 0.70, not the table top of 0.60. The source comment says table
height and is wrong by 0.10 m. So no goal is ever on the table.

That write happens AFTER the clamp, so the z bound is not an invariant of the
goal state for the forced-flat fraction.
"""
...
def quat_from_euler_xyz(roll, pitch, yaw):
    """isaacgym.torch_utils.quat_from_euler_xyz, returns xyzw."""
...
def reset_goal_pose_init(object_pos, object_rot, goal_displacement):
    """The opening goal. A pure translation, orientation copied, no clamp."""
