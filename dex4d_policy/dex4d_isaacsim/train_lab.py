"""Train Dex4D on IsaacLab with Dex4D's own PPO, unmodified.

The RL stack has zero isaacgym imports, 2,418 lines of pure torch, so it is
imported straight from the repo rather than reimplemented. `Dex4DVecAdapter`
presents the lab env with the `VecTaskPython` surface it expects.

Isaac Sim does not exit on close, measured, so this force-exits.
"""
import argparse
...
def set_seed(seed):
    """`utils/config.py:35-59`, the non-deterministic branch, which is what both
    launchers take because neither passes `--torch_deterministic`. Without this
    the lab arm was fully unseeded while the gym arm ran `--seed=0`, so the two
    were not comparable and neither could be repeated."""
    import random
...
def main():
    ...
    # ppo.py -> utils/result_recorder.py -> `from isaacgym import gymapi`, and
    # isaacgym cannot live in an Isaac Sim process. The stub refuses on every
    # attribute, so the shared PPO imports and any real reach into gymapi
    ...
    # Same construction as utils/process_sarl.py:19, same defaults, so the two
    # backends train with an identical PPO configuration.
    runner = PPO(
        vec_env=vec,
