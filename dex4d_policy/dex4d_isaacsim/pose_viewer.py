"""Pose-only interactive HTML capture, logged to wandb.

A gym.Wrapper that samples one env's state tensors each step and
periodically writes a Three.js URDF viewer. No cameras, no Replicator.
"""
...
# The browser fetches robot meshes from a public repo carrying the same mesh
# tree. The robot URDF is read off local disk and its refs rewritten.
ROBOT_RAW_BASES = {
...
def _obj_to_stl_data_uri(obj_path):
    """Embedded meshes travel as STL, the template's OBJ branch rejects
    data URIs."""
    vertices, faces = [], []
...
def _embed_object_meshes(urdf_text, urdf_dir, strip_materials=False):
    """Inline every mesh as a data URI.

    strip_materials drops every <material> so a colour override survives.
    """
    root = ET.fromstring(urdf_text)
    if strip_materials:
        # urdf-loader re-applies a material after loadMeshCb returns, and with
        # none present it applies a grey default. Write GOAL_COLOR in as a
        # real material so that re-application lands on the right colour.
        r, g, b = GOAL_COLOR
...
def _rewrite_robot_mesh_urls(urdf_text, raw_base):
    """Point relative mesh refs at the raw base."""
    root = ET.fromstring(urdf_text)
    for mesh_elem in root.findall(".//mesh"):
        filename = mesh_elem.get("filename")
        if not filename or filename.startswith(("http://", "https://", "data:")):
...
def table_urdf_text(dims) -> str:
    """The table is built in code, so its urdf is emitted from the env dims."""
    dx, dy, dz = dims
    return (
        '<?xml version="1.0"?>\n<robot name="table">\n  <link name="box">\n'
...
def capture_pose_viewer_frame(env, env_id: int) -> dict[str, Any]:
    ...
    # Canonical joint order, so the viewer's urdf names line up.
...
def build_pose_viewer_html(*, frames, robot_urdf_text, object_urdf_text,
    ...
    """Embed the robot, object, table and goal urdfs into one html page.

    Mesh filenames are rewritten to raw urls so the browser can fetch them.
    The goal is the object urdf tinted green.
    """
...
class Dex4DPoseViewerWrapper(gym.Wrapper):
    ...
    Uses a separate RNG so viewer env sampling never perturbs the training
    seed, and rebinds at the start of every capture window.
    """
    ...
    def __init__(self, env, *, output_dir, capture_len: int, capture_interval: int,
        ...
        # Local urdf, refs rewritten, never fetched.
        ...
        # Separate RNG, so the sampled env sequence is reproducible.
    ...
    def _bind_env(self, env_id: int) -> None:
        ...
        # Object keeps its materials, goal sheds them so the tint wins.
