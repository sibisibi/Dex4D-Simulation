"""Dex4D's observation, engine agnostic. `compute_full_state` at :924.

The layout is a running pointer, not a table, so it is reproduced as a pointer
here rather than tidied into slices. Getting the order wrong is silent, the
network just learns garbage, so the layout is asserted against the source's own
totals in `layout()`.

  states_buf   3*19 dof + 65 fingertip states + 30 fingertip F/T + 13 palm
               + 19 actions + 20 object and goal + 384 keypoints + 64 visual
               + 15 fingertip-to-object  =  283 + num_kp_flatten
  obs_buf      the same when symmetric. When asymmetric it is 2*19 dof
               + 19 actions + masked keypoints only.

Two things carried across deliberately.

Absolute world keypoints go into the buffer un-normalised, and the teacher's
first layer is nn.Linear(6, 128) with no centering. That is why a goal-box change
wants a fresh run rather than a warm start.

`max_mask_height` is a per-env quantile fraction of the object's own keypoint z
spread, not an absolute height. The name is misleading and the behaviour is
scale free.
"""
...
VEL_OBS_SCALE = 0.2            # :57
FORCE_TORQUE_OBS_SCALE = 10.0  # :58


def layout(num_robot_dofs: int, num_fingertips: int, num_keypoints: int,
           kp_downsample_ratio: int, asymmetric: bool) -> dict:
    """Sizes and offsets, derived exactly as the source derives them."""
    ...
    for name, size in (("dof", 3 * num_robot_dofs),
        ...
        off[name] = (p, size)
        p += size
    assert p == num_states, f"state layout sums to {p}, source says {num_states}"
...
def unscale(x, lower, upper):
    """isaacgym.torch_utils.unscale, [lower, upper] onto [-1, 1]."""
    return (2.0 * x - upper - lower) / (upper - lower)


def keypoint_local_to_world(keypoints, object_pos, object_rot):
    """utils/util.py:219. xyzw rotation applied to (N, K, 3) local keypoints."""
    q = object_rot
...
def fingertip_to_object_vecs(fingertip_pos, keypoints):
    """utils/util.py:413. Vector from each fingertip to its NEAREST object
    keypoint, not to the object centre. The port used the centre, which is a
    different quantity in those 15 channels."""
...
def write_states(buf, lay, *, dof_pos, dof_vel, dof_force, dof_lower, dof_upper,
    ...
    """Fills states_buf in the source's own order. Returns object and goal keypoints."""
    o = lay["offsets"]
