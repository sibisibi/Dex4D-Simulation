"""Dex4D's reward, engine agnostic.

Every term is a straight translation of `tasks/fr3_xhand_ap2ap.py:1476-1546`.
Pure torch so the parity harness can run this and the gym original side by side.

Two things carried across deliberately.

There is no lifting term. Dex4D has twelve reward functions and none raises the
object. The only `hand_up` term lives inside the `goal_cond` branch, which is
asserted off for FR3 plus XHand, so it is not ported. The fixed 0.20 m first
goal is what teaches lifting.

`_prepare_reward_function` pops any scale that is zero, so a zeroed term is
deleted from the loop rather than multiplied by zero. `active_terms` reproduces
that, which matters because stage 3 zeroes obj_finger and obj_hand.
"""
from __future__ import annotations

import torch

# tasks/fr3_xhand_ap2ap.py:223-225. The yaml successTolerance is dead config.
ACHIEVED_LAST_TIME = 30
SUCCESS_THRESHOLD = 0.05
TABLE_HEIGHT_TOLERANCE = 0.62      # :1505, table top is 0.60


def contact_flag(obj_finger_dist: torch.Tensor,
                 obj_hand_dist: torch.Tensor) -> torch.Tensor:
    """:1440. Sum over five fingers under 0.6, palm under 0.12. The paper states
    four fingers under 0.48, the same 0.12 per finger carried to five."""
...
def reward_goal_obj(goal_obj_dist: torch.Tensor, flag: torch.Tensor) -> torch.Tensor:
    """A level, not a progress delta. Crosses zero at d = 0.4667, so it pays for
    holding the object near the goal. SimToolReal's max(d* - d, 0) does not."""
...
def reward_table_collision_penalty(fingertip_pos: torch.Tensor,
    ...
    """Constant upward pressure. Fires below 0.62 with the table top at 0.60."""
...
def reward_fall_penalty(goal_obj_dist: torch.Tensor, too_far: float) -> torch.Tensor:
    """Scale is zero in every shipped config, so this never contributes."""
...
def reward_dof_acc(last_dof_vel: torch.Tensor, dof_vel: torch.Tensor,
    ...
    """:1538 forms the acceleration inside the term, so the signature takes the
    two velocities and dt rather than a precomputed acceleration."""
...
"""All twelve gym terms. `_get_rewards` raises on anything outside it,
rather than skipping an active term in silence."""


def active_terms(reward_scales: dict) -> dict:
    """`_prepare_reward_function` at :1366 pops zeros rather than scaling by them."""
